  ...
  try {
    ...
    const [
      ...
      Promise.all([
        ...
        prisma.payment.count({ where: { provider: "stripe", paymentStatus: "succeeded" } }),
        prisma.payment.count({ where: { provider: "stripe", paymentStatus: "refunded" } }),
        prisma.payment.count({ where: { provider: "stripe", paymentStatus: "partially_refunded" } }),
