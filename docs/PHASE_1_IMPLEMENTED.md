...
| **P2-G — Feature analytics** | **`/admin/features?tab=analytics`** + **`?inactiveDays=`** (7–365, UI pills 7/30/90). **`GET /api/admin/feature-analytics/export`** CSV (BOM). `featureAnalyticsSnapshot`, `featureAnalyticsSnapshotToCsv`, `parseFeatureAnalyticsInactiveDays`. **Still open:** time-series, cohort churn. | **Done (v1–v2)** |
| **P5-E — Feature control hub (partial)** | `/admin/features` — Overview + Analytics (window + CSV export); hub stats + directory; **Feature bundles** links. Promos / inline edits / charting still open. | **Done (v1–v5)** |
