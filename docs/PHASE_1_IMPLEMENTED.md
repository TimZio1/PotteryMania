...
| **P2-F — Feature billing (partial)** | Per-feature + bundle Checkout + webhooks. **Cancel:** immediate (`subscriptions.cancel`) clears shared sub rows. **Period-end cancel:** vendor `POST /api/studios/[id]/feature-requests` with `cancelAtPeriodEnd: true` → Stripe `cancel_at_period_end`, `pending_cancel` + `deactivatesAt`; resume by turning add-on on again; **`customer.subscription.updated`** keeps DB in sync; event kind **`vendor_cancel_at_period_end`** (ledger net neutral). **Still open:** proration, subscription items per feature. | **Partial** |
...
| **P5-E — Feature control hub (partial)** | `/admin/features` — Overview + Analytics: **SKU editor** deep links + row highlight; **inline PATCH** for **list price**, **sort order** (vendor catalog order), **catalog on/off**, **visibility**, **grant by default**; snapshot CSV adds **sort_order** plus prior catalog columns. Events CSV + charts unchanged. **Still open:** promos / campaign pricing. | **Done (v1–v17 hub PATCH + CSV)** |
