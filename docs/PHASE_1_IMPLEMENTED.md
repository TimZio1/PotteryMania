...
| **P5-E — Feature control hub (partial)** | `/admin/features` — Overview + Analytics: **SKU editor** deep links + row highlight; **inline PATCH** for **list price** (blur/Enter), **catalog on/off**, and **visibility** without leaving the hub; snapshot CSV includes **list_price_cents** + **currency**. Events CSV + charts unchanged. **Still open:** promos / campaign pricing. | **Done (v1–v15 hub PATCH + CSV)** |
