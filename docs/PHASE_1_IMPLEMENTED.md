...
| **P5-E — Feature control hub (partial)** | `/admin/features` — Overview + Analytics: **SKU editor** deep links + row highlight; **inline PATCH** for **list price**, **catalog on/off**, **visibility**, and **grant by default**; snapshot CSV includes **list_price_cents**, **currency**, **grant_by_default**. Events CSV + charts unchanged. **Still open:** promos / campaign pricing. | **Done (v1–v16 hub PATCH + CSV)** |
