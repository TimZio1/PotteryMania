...
| **P2-G — Feature analytics** | **`/admin/features?tab=analytics`** + **`?inactiveDays=`** (7–365, UI pills 7/30/90). **`GET /api/admin/feature-analytics/export`** CSV (BOM). `featureAnalyticsSnapshot`, `featureAnalyticsSnapshotToCsv`, `parseFeatureAnalyticsInactiveDays`. **v3:** `featureActivationAdminAuditDailySeries` — UTC daily bars for hyperadmin **grants** vs **revokes** (`admin_audit_logs`, `reason` status:active / inactive); respects feature directory filter. **Still open:** vendor/Stripe event series, true cohort churn. | **Done (v1–v3 partial)** |
| **P5-E — Feature control hub (partial)** | `/admin/features` — Overview + Analytics (window + CSV + **audit time-series charts**); hub stats + directory; **Feature bundles** links. Promos / inline PATCH on hub still open. | **Done (v1–v5 + charts)** |
