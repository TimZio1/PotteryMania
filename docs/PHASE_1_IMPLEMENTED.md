...
| **P2-G — Feature analytics** | **`/admin/features?tab=analytics`** + **`?inactiveDays=`** + CSV + audit + lifecycle charts. **Append-only ledger:** `StudioFeatureActivationEvent` + migration `20260409100000_studio_feature_activation_events`; `recordStudioFeatureActivationEvent` on vendor API, admin PATCH, Stripe checkout (`checkout_single` / `checkout_bundle`), `markActivationsEndedForStripeSubscription` (`stripe_subscription_ended`). **`featureActivationEventLedgerDailySeries`** — checkout vs sub-end charts. **Still open:** webhook idempotency keys, true cohort churn. | **Done (v1–v5 partial)** |
| **P5-E — Feature control hub (partial)** | `/admin/features` — Overview + Analytics (window + CSV + **audit + lifecycle charts**); hub stats + directory; **Feature bundles** links. Promos / inline PATCH on hub still open. | **Done (v1–v5 + charts)** |
