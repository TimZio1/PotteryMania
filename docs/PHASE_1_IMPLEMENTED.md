...
| **P5-J — Admin audit (partial)** | `PATCH /api/admin/platform-features/[id]`: optional **`reason`** (≤500 chars) on audit rows; **no-op** identical values → no DB write / no duplicate `platform_feature.update` log; **`before`/`after`** include **description**, **category**, **currency**. Full audit UI presets + mandatory reason modals still open. | **Partial (v1)** |
