...
| **P2-G — Feature analytics** | **`/admin/features?tab=analytics`** + **`?inactiveDays=`** (7–365, UI pills 7/30/90). **`GET /api/admin/feature-analytics/export`** CSV (BOM). `featureAnalyticsSnapshot`, `featureAnalyticsSnapshotToCsv`, `parseFeatureAnalyticsInactiveDays`. **v3:** `featureActivationAdminAuditDailySeries` (hyperadmin grant/revoke from audit). **v4:** `featureActivationLifecycleDailySeries` — new activation **rows** by `createdAt` + **turn-on** counts by `activatedAt` (vendor + Stripe webhook + admin). **Still open:** immutable Stripe event log, true cohort churn. | **Done (v1–v4 partial)** |
