...
| **P2-G — Feature analytics** | **`/admin/features?tab=analytics`** + **`?inactiveDays=`** + CSV + audit + lifecycle charts. **Ledger:** `StudioFeatureActivationEvent` (`20260409100000`); **`stripe_checkout_session_id`** + unique `(feature_id, stripe_checkout_session_id)` (`20260409110000`) — checkout events **upsert** so Stripe webhook retries do not duplicate. **`featureActivationEventLedgerDailySeries`**. **Still open:** `subscription.deleted` dedupe, true cohort churn. | **Done (v1–v5 partial)** |
