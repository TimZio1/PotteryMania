...
| **P2-G — Feature analytics** | **Ledger** + migrations `20260409100000`–`20260409120000`. Analytics tab: **Snapshot CSV** (`/api/admin/feature-analytics/export`) + **Events CSV** (`/api/admin/feature-activation-events/export`, `?inactiveDays=`, optional `?featureId=`, max 25k rows, UTF-8 BOM). `lib/admin-feature-activation-events-export.ts`. **Still open:** cohort churn. | **Done (v1–v5 partial)** |
