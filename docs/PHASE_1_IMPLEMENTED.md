...
| **P5-E — Feature control hub (partial)** | `/admin/features` — Overview + Analytics: **SKU editor** column → `/admin/platform-features#catalog-feature-{id}`; catalog table rows are anchor targets with scroll + ring highlight. Snapshot + events CSV + charts. **Still open:** promos, true inline PATCH without leaving hub. | **Done (v1–v5 + charts + deep links)** |
