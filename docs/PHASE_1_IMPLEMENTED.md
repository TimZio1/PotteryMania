...
| **P5-J — Admin audit (partial)** | `PATCH` + **`POST /api/admin/platform-features`**: optional **`reason`**; **no-op PATCH** skips write/log; richer **`platform_feature.*`** JSON. **`/admin/audit`**: **Quick filters** (catalog SKUs, bundles, add-on admin, studios, users, orders, Stripe refunds, reset). **`/admin/platform-features`**: optional **audit note** on all saves. Mandatory reason modals still open. | **Partial (v1–v2)** |
